...
switch ($op) {
    ...
    case 'regenerate_security_token':
        $token_raw = sha1(
                $vbulletin->userinfo['userid'] .
                         sha1($vbulletin->userinfo['salt']) . sha1(COOKIE_SALT));
    ...
    case 'complete_your_profile':
        ...
        if ($vbulletin->options['avatarenabled']) {
            if ($avatar_usergroup_enabled) {
                ...
                if (empty($vbulletin->GPC['upload'])) {
                    ...
                    if (! preg_match("/http/i",
                            $vbulletin->options['site_registration_includes_path_img'])) {
                        $vbulletin->GPC['avatarurl'] = $vbulletin->options['bburl'] .
                                 "/includes/site-registration/img/unknown.png";
                    } else {
                        $vbulletin->GPC['avatarurl'] = $vbulletin->options['site_registration_includes_path_img'] .
                                 "/img/unknown.png";
                    }
                }
            ...
            // predefined avatar
            $userpic = &datamanager_init('Userpic_Avatar', $vbulletin,
                    ERRTYPE_CP, 'userpic');
            ...
            if (empty($vbulletin->GPC['parent-guardian-email'])) {
                ...
                if (is_valid_email_address(
                    ...
                    if (! checkdnsrr($email_domain, "MX")) {
                        ...
                        // fetch_error('fieldmissing_parentemail')
    ...
    ");
        ...
        if (fetch_require_hvcheck('register')) {
            ...
            if (strtoupper($vbulletin->GPC['security_code']) != strtoupper(
        ...
        if ($valid_entries) {
            ...
            $userdata->set('password',
                    ($vbulletin->GPC['password_md5'] ? $vbulletin->GPC['password_md5'] : $vbulletin->GPC['password']));
            ...
            // set user title
            $userdata->set_usertitle('', false,
                    $vbulletin->usergroupcache["$newusergroupid"], false, false);
            ...
            if ($vbulletin->options['reqbirthday'] ||
                ...
                $coppaage = $vbulletin->input->clean_gpc('c',
                        COOKIE_PREFIX . 'coppaage', TYPE_STR);

                if (! empty($coppaage)) {
                    if ($vbulletin->options['usecoppa'] and
                             $vbulletin->options['checkcoppa'] and $coppaage) {
            ...
            // $userdata->set('timezoneoffset',
            // $vbulletin->GPC['timezoneoffset']);

            if ($_SESSION['site_registration']['coppauser'] === true &&
                     $vbulletin->options['usecoppa'] > 0) {
                $userdata->set_info('coppauser',
                        $_SESSION['site_registration']['coppauser']);
            ...
            if (! empty($userdata->errors)) {
                ...
                if ($vbulletin->options['verifyemail']) {
                    $activateid = build_user_activation_id($userid,
                            (($vbulletin->options['moderatenewmembers'] or
                                     $_SESSION['site_registration']['coppauser']) ? 4 : 2),
                                    0);
    ...
    case 'create_site_account_first_step':
            ...
            if (empty($vbulletin->GPC['birthdate'])) {
                ...
                if (check_date($vbulletin->GPC['birthdate'])) {
                    ...
                    if ($month == 0 or $day == 0 or
                             ! preg_match('#^\d{4}$#', $year) or $year < 1901 or
                             $year > $current['year']) {
                        $valid_entries = FALSE;
                        $messages['errors'][] = $message = fetch_error(
                    ...
                    if ($year < 1970 or (mktime(0, 0, 0, $month, $day, $year) <= mktime(
                            0, 0, 0, $current['month'], $current['day'],
                            $current['year'] - 13))) {
        ...
        if ($valid_entries) {
            ...
            $temp_table_query = "
            CREATE TABLE IF NOT EXISTS " . TABLE_PREFIX . "siteregistration_temp (
            ...
            $vbulletin->db->query_write(
                    "
            REPLACE INTO " . TABLE_PREFIX . "siteregistration_temp
    ...
    default:
        ...
        // check if variables are set
        if (empty($vbulletin->GPC['vb_login_username']) or
                 empty($vbulletin->GPC['vb_login_password']) or
                 $vbulletin->GPC['vb_login_password'] == md5("")) {
            $valid_login = FALSE;
            // $userdata->error('enter_password_for_account');
            ...
            $vbulletin->GPC['vb_login_md5password'] = $vbulletin->GPC['vb_login_md5password_utf'] = ($vbulletin->GPC['vb_login_password']);
            ...
            if (! verify_authentication($vbulletin->GPC['vb_login_username'],
                ...
                if ($vbulletin->GPC['logintype'] === 'cplogin' or
                         $vbulletin->GPC['logintype'] === 'modcplogin') {
    ...
    ");
        ...
        if (is_banned_email($vbulletin->GPC['email'])) {
            if (! $vbulletin->options['allowkeepbannedemail']) {
                $valid_entries = FALSE;
                $messages['errors'][] = $message = $error = fetch_error(
                        "banemail");
                $messages['fields'][] = $error_type = "email";
            }
        }
        ...
        if ($valid_entries) {
            ...
            if ($fbID) {
                ...
                if ($result) {
                    ...
                    // start new session
                    $vbulletin->userinfo = $vbulletin->db->query_first(
                            "SELECT " . TABLE_PREFIX . "vbnexus_user.userid, " .
                                     TABLE_PREFIX . "user.password FROM " .
                                     TABLE_PREFIX . "vbnexus_user
                                                        INNER JOIN " .
                                     TABLE_PREFIX . "user ON " . TABLE_PREFIX .
                                     "user.userid = " . TABLE_PREFIX . "vbnexus_user.userid
                                                        WHERE nonvbid = " .
                                     $fbID);
    ...
    case "linkaccount":
        ...
        if ($valid_entries) {
            ...
            $password = $vbulletin->db->escape_string(
                    $vbulletin->GPC['password']);
                ...
                if ($dbPassword != $password) {
                    ...
                    if (strlen($fbID) > 1) {
                        ...
                        if (! empty($activationid)) {
                            ...
                            vbsetcookie('userid',
                                    $vbulletin->userinfo['userid'], true, true,
                                    true);
                            vbsetcookie('password',
                                    md5(
